...
const getAmazonRanking = async function (
  searchURL,
  ...
  for (const { keyword, itemId } of items) {
    ...
    const now = new Date()
    const japanNow = new Date(now.getTime() + (now.getTimezoneOffset() + (9 * 60)) * 60 * 1000)
    const date = dateformat(japanNow, 'yyyy/mm/dd')
...
exports.amazon = async ({ data }) => {
  const { name, id: spreadsheetId } = data
  console.log(`${name} - Amazon - START`)
